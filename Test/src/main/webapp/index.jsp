	...
	<script type="text/javascript">
		...
		function build_emps_table(result){
			...
			$.each(emps,function(index,item){
				...
				//连接查询职位名称
				//vardIdTd = $("<td></td>").append(item.dId);
				// <button class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>编辑</button>

				//编辑和删除操作
				var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm btn-sm edit_btn")
						.append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("编辑");
				//为编辑按钮添加一个自定义属性，来表示当前员工id
				editBtn.attr("edit-id",item.empId);
				var delBtn = $("<button></button>").addClass("btn btn-danger btn-sm btn-sm delete_btn")
		...
		function getDepts(ele){
			...
			$.ajax({
				...
				type:"GET",
				success:function(result){
					//console.log(result);看请求信息是否有
					//"extend":{"depts":[{"deptId":1,"deptName":"开发部"},{"deptId":2,"deptName":"测试部"}]}}console.log(result);
					//显示部门信息在下拉列表中
